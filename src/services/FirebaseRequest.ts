...
export async function callGetQuestions(): Promise<QuestionsFormatter[]> {
    ...
    let targetAuience = `https://${process.env.GOOGLE_CLOUD_REGION}-${process.env.GOOGLE_PROJECT_ID}.cloudfunctions.net/${process.env.ROUTE_GET_QUESTIONS}`;
    if(process.env.NODE_ENV == 'development') {
        targetAuience = process.env.API_URL + "/getQuestions" || targetAuience;
    }
